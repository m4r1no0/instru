...
# OBTENER POR ID
# =====================================
@router.get("/{id_supervisor}", response_model=SupervisorOut)
def get_supervisor(id_supervisor: int, db: Session = Depends(get_db)):
    supervisor = supervisor_crud.get_supervisor_by_id(db, id_supervisor)
...
def get_all_supervisores(db: Session = Depends(get_db)):
...
def update_supervisor(
    id_supervisor: int, supervisor: SupervisorUpdate, db: Session = Depends(get_db)
):
    updated = supervisor_crud.update_supervisor(db, id_supervisor, supervisor)
...
def delete_supervisor(id_supervisor: int, db: Session = Depends(get_db)):
    deleted = supervisor_crud.delete_supervisor(db, id_supervisor)
