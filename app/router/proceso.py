...
def create_proceso(proceso: ProcesoCreate, db: Session = Depends(get_db)):
    if not proceso_crud.create_proceso(db, proceso):
        raise HTTPException(status_code=400, detail="No se pudo crear el proceso")
...
# LISTAR POR CONTRATO
# =====================================
@router.get("/contrato/{id_contrato}", response_model=List[ProcesoOut])
def get_procesos_by_contrato(id_contrato: int, db: Session = Depends(get_db)):
    return proceso_crud.get_procesos_by_contrato(db, id_contrato)
...
def update_proceso(
    id_proceso: int, proceso: ProcesoUpdate, db: Session = Depends(get_db)
):
    updated = proceso_crud.update_proceso(db, id_proceso, proceso)
...
def delete_proceso(id_proceso: int, db: Session = Depends(get_db)):
    deleted = proceso_crud.delete_proceso(db, id_proceso)
