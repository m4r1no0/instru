...
def create_poliza(poliza: PolizaCreate, db: Session = Depends(get_db)):
    if poliza_crud.get_poliza_by_numero(db, poliza.numero_poliza):
...
# =====================================
@router.get("/instructor/{id_instructor}", response_model=List[PolizaOut])
def get_polizas_por_instructor(id_instructor: int, db: Session = Depends(get_db)):
    return poliza_crud.get_polizas_by_instructor(db, id_instructor)
...
def update_poliza(id_poliza: int, poliza: PolizaUpdate, db: Session = Depends(get_db)):
    updated = poliza_crud.update_poliza(db, id_poliza, poliza)
...
def delete_poliza(id_poliza: int, db: Session = Depends(get_db)):
    deleted = poliza_crud.delete_poliza(db, id_poliza)
